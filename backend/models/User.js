...
const userSchema = new mongoose.Schema(
  {
    phone: {
      ...
      unique: true,
      match: [/^\+91[6-9][0-9]{9}$/, "Phone must be a valid Indian number starting with +91"],
    },
    username: {
      type: String,
    ...
    vendorRequest: {
      ...
      phone_whatsapp: { type: String, match: [/^\+91[6-9][0-9]{9}$/, "Valid Indian number"] },
      whatsapp_number: { type: String, match: [/^\+91[6-9][0-9]{9}$/, "Valid Indian number"] },
      category: {
        type: String,
        enum: [
          "Wedding Venues",
